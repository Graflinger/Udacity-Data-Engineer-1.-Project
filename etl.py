...
def process_song_file(cur, filepath):
    ...
    # insert song record
    song_data = song_data = list(df[["song_id", "title", "artist_id", "duration"]].values[0])
...
def process_log_file(cur, filepath):
    ...
    for index, row in df.iterrows():
        ...
        songplay_data = (datetime.datetime.fromtimestamp(row.ts/1000.0), row.userId, row.level, songid, artistid, row.itemInSession, row.location, row.userAgent)
